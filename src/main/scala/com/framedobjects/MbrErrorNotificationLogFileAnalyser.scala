...
object MbrErrorNotificationLogFileAnalyser {
  ...
  def main(args: Array[String]) {
	 val fileName = "/users/jensr/Documents/DevNotes/investigations/sc-2666/08012015/.."
  }
  ...
  def processFile(fileName: String, startDate: Date, endDate: Date) {
    ...
    val advertRDD = notificationRDD.filter(_.winningAdvertId == 139448l)
    val winningAdvertRDD = notificationRDD.filter(_.win)
